...
class TrainManager(object):
    ...
    def validate(self, data_loader, step=0):
        ...
        with torch.no_grad():
            for sample in data_loader:
                ...
                *cls_logits_list_2, DIV2_2, U1_2, U2_2, W1_2, W2_2 = self.model2(image)
                *cls_logits_list_3, DIV2_3, U1_3, U2_3, W1_3, W2_3 = self.model3(image)

                
                pred_count = DIV2.cpu().numpy().sum()
                pred_count_2 = DIV2_2.cpu().numpy().sum()
                pred_count_3 = DIV2_3.cpu().numpy().sum()
                diff_1 = pred_count - gt_count
                diff_2 = pred_count_2 - gt_count
                diff_3 = pred_count_3 - gt_count
                #print ("Diff  :  "  + str(abs(diff_1)) + "   " + str(abs(diff_2)) + "  " + str(abs(diff_3)))
                new_density_map = (DIV2.cpu().numpy() + DIV2_2.cpu().numpy()+ DIV2_3.cpu().numpy())/3
                new_count = new_density_map.sum()
                print ("Count :  "  + str(pred_count) + "   " + str(pred_count_2) + "  " + str(pred_count_3) + "   " + str(new_count) + "   " +str(gt_count))
                diff = new_count - gt_count
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.train.start_epoch, self.cfg.train.num_epochs):
            ...
            if epoch % self.cfg.validation.save_ckpt_every_epochs == 0:
                ...
                if not os.path.isdir(pjn(self.tbx_wrtr_dir, 'checkpoints_'+self.cfg.dataset.part)):
                    os.mkdir(pjn(self.tbx_wrtr_dir, 'checkpoints_'+self.cfg.dataset.part))
                self.save_ckpt(
                    epoch=epoch,
                    fpath=pjn(self.tbx_wrtr_dir, 'checkpoints_'+self.cfg.dataset.part, nm))
...
@hydra.main(config_path="/content/S-DCNet/conf", config_name="config_train_val_test")
def main(cfg):
    """
    ...
    """
    print(cfg)
    #with open(r'/content/S-DCNet/conf/config_train_val_test.yaml') as file:
    #    cfg = yaml.load(file)
    #hydra.main(config_path="/content/S-DCNet/conf/config_train_val_test.yaml")
    #print(cfg)
    ...
    if cfg.train.pretrained_ckpt:
        ...
        cfg.train.start_epoch = loaded_struct['epoch']+1
    ...
    additional_cfg['tbx_wrtr_dir'] = '/content/drive/MyDrive/MastersResearch/Codes/S-DCNet/S-DCNet-2'
