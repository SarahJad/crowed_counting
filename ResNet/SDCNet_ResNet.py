...
class SDCNet(nn.Module):
    ...
    def __init__(
        ...
        # ResNetLayers
        self.layer1 = self.make_resnet_layers(num_layers, layers[0], intermediate_channels=64, stride=1)
        self.layer2 = self.make_resnet_layers(num_layers, layers[1], intermediate_channels=128, stride=2)
        self.layer3 = self.make_resnet_layers(num_layers, layers[2], intermediate_channels=256, stride=2)
        self.layer4 = self.make_resnet_layers(num_layers, layers[3], intermediate_channels=512, stride=2)

        self.label2count_tensor = torch.tensor(
            label2count_list, dtype=torch.float32)

        num_classes = len(label2count_list)
        self.count_interval_classif = fully_conv_classif(2048, num_classes)

        ## upsampling (UNet-like)
        self.up_from_5_to_4 = Up(
            up_in_ch=2048, up_out_ch=1024, cat_in_ch=(0+2048), cat_out_ch=2048)

        self.division_decider = fully_conv_classif(2048, 1)
        self.supervised = supervised
        self.upsampler = fully_conv_classif(2048, 1)
        
        self.up_from_4_to_3 = Up(
            up_in_ch=2048, up_out_ch=1024, cat_in_ch=(512+1024), cat_out_ch=2048)

        self._initialize_weights()

        if load_pretr_weights_vgg:
            pretr_dict = torchvision.models.resnet50(pretrained=True).state_dict()
    ...
    def make_resnet_layers(self, num_layers, num_residual_blocks, intermediate_channels, stride):
        layers = []
