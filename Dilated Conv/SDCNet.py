...
def make_layers_vgg(cfg, in_ch=3, use_batch_norm=False, dilated=False):
    """
    ...
    for v in cfg:
        if v == 'M':
            ...
            if dilated == True :
                conv2d = nn.Conv2d(in_ch, v, kernel_size=3, padding=2, dilation=2)
            else:
...
class SDCNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1_features = make_layers_vgg([64, 64, 'M'], in_ch=3, dilated=True)
        self.conv2_features = make_layers_vgg([128, 128, 'M'], in_ch=64, dilated=True)
        self.conv3_features = make_layers_vgg([256, 256, 256, 'M'], in_ch=128, dilated=True)
        self.conv4_features = make_layers_vgg([512, 512, 512, 'M'], in_ch=256, dilated=True)
        self.conv5_features = make_layers_vgg([512, 512, 512, 'M'], in_ch=512, dilated=True)
