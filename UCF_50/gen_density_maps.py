...
def check_consist_imgs_annots(imgs_dir, annot_dir):
    ...
    print(len(jpg_basenames))
    print(len(mat_basenames))
    print(jpg_basenames)
    print(mat_basenames)
...
def get_headpoints_dict(annot_dir):
    ...
    for f, bn in zip(mat_files, mat_basenames):
        ...
        ####print(mat["annPoints"].shape)
        #numpy_void_obj = mat['image_info'][0][0][0][0]
        headpoints = mat["annPoints"]
...
def generate_density_maps(
    ...
    for bn, points in basename2headpoints_dict_part.items():
        img_fpath = pjn(imgs_dir, bn[:-4] + '.jpg')
        print(bn)
...
def main():
    with open(r'/content/S-DCNet/config_density_maps.yaml') as file:  
        cfg = yaml.load(file)
    for t in ['train_data', 'test_data']:
        the_dir = pjn(
            ...
            t)
        the_dir = "/content/drive/MyDrive/MastersResearch/Dataset/UCF_CC_50/test_data"
